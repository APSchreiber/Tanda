...
con.execute("CREATE TABLE circles_people (circleid INTEGER NOT NULL, personid INTEGER NOT NULL, PRIMARY KEY(circleid, personid))")
con.execute("INSERT INTO circles_people (circleid, personid) VALUES (1, 1)")
...
con.execute("CREATE VIEW payout_schedules_vw AS SELECT participants_vw.personid, participants_vw.first, participants_vw.last, participants_vw.name, participants_vw.circleid, participants_vw.capacity, participants_vw.enrolled, participants_vw.loan, payout_schedules.payout_order, (participants_vw.loan / participants_vw.capacity) as payment_amount, (date(participants_vw.start, '+' || payout_schedules.payout_order || ' month')) as payout_date, (participants_vw.loan / (SELECT COUNT(*) FROM payout_schedules ps2 WHERE ps2.person = payout_schedules.person AND payout_schedules.circle = ps2.circle)) as payout_amount, (SELECT COUNT(*) FROM payout_schedules ps2 WHERE ps2.person = payout_schedules.person AND payout_schedules.circle = ps2.circle) as split_num FROM payout_schedules LEFT JOIN participants_vw ON participants_vw.personid = payout_schedules.person AND participants_vw.circleid = payout_schedules.circle")
